---
title: "Bio 720 In Class Exercise Strings"
...
```{r}
table(stri_isempty(species_text))
```
...
```{r}
length(grep("introduction", species_text, ignore.case = TRUE))
```


How would you check the lines?
```{r}
grep("introduction", species_text, ignore.case = TRUE, value = TRUE)
```
...
species_slice <- species %>%
  slice(grep("^INTRODUCTION\\.", text):(grep("^INDEX\\.", text))-1) #Slice all lines except from Introduction to (:) Index
...
table(nzchar(species_slice$text))
```
...
```{r}
species_tidy <- species_filtered %>% #uNNEST EVERYTHING AND TURN EVERYTHING INTO A LIST OF WORDS 
...
```{r}
species_tidy <- species_tidy %>% #STOP WORDS 
...
```{r}
library(dplyr)
species_tidy %>% count(word, sort = TRUE)
...
wordcloud(species_tidy$word, max.words = 50, random.order = FALSE)
```
