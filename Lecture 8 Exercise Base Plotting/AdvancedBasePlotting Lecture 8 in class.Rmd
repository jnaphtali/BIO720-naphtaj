...
```{r}
plot(y ~ x, pch= 20, type = "n") #type = n tells R to plot null 

polygon(x = c(1:nrow(new), nrow(new):1), #polygons have two faces, the face that increases (x = c(1:nrow)) and the face that decreases (y=c(pred[,2])) 
...
```{r}
col2rgb("red") #convert red to rgb value 
...
```{r}
old_par <- par() #save old_par graph space drawing into its own variable so you can quickly revert back to it
