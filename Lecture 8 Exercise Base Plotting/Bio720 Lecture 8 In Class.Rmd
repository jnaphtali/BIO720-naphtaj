...
plot(femur ~ tibia,
     ...
     data = dll_data_subset,
     bty = "n") ##boxtype, or the borders around the graph 
```
...
plot(femur ~ tibia,
     ...
     bty = "n") ##boxtype, or the borders around the graph 
```  
Changing the size of the points to 0.5 or 0.8 alleviates the overlapping of points. we can change the opacity of the points too. 
...
plot(SCT ~ tibia,
     ...
     bty = "n") ##boxtype, or the borders around the graph 
```  
Nothing is plotted because SCT is an integer
...
Set the `factor` argument in the jitter function higher to increase random noise generated. 
...
boxplot(SCT ~ genotype, 
        ...
        ylab = " # SCT",
        notch = TRUE,
        varwidth = TRUE) #this width argument makes the sizes of boxes drawn to be proportional to the number of observations in groups
```
...
1- How might you improve this boxplot to make the raw observations a bit clearer? Have to add jitter in the x axis and Y. 
```{r}
boxplot(SCT ~ genotype, 
        data  = dll_data_subset,
        ylab = " # SCT",
...
hist(SCT_wt$SCT,
     ...
     breaks=seq(min(SCT), max(SCT)), ##Make sure breaks are consistent with the breaks from the first histogram.
     add = TRUE)
