...
Because the former solution assigned a value to x in the global environment outside of the function, while the latter solution applied a value within the countFun function. 
...
Pre-allocating memory by assigning a vector with correct length in the environment is quicker than allowing the function to dynamically allocate and increase memory usage as the function runs. 
...
R computes faster when performing operations over a vector. This is due to the low-level language that R was built upom (fortran or c)

So what is the lesson learned from this?

Try to use vectors as much as a possible than loops when computing with R. 

--- Completed exercise up to here.---
